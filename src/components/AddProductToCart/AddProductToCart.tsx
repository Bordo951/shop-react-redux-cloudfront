...
export default function AddProductToCart({ product }: AddProductToCartProps) {
  const { data = { cart: { items: [] } }, isFetching } = useCart();
  const { mutate: upsertCart } = useUpsertCart();
  const invalidateCart = useInvalidateCart();
  const cartItem = data.cart.items.find((i) => i.productId === product.id);

  const addProduct = () => {
    upsertCart(
      [{ productId: product.id as string, count: cartItem ? +cartItem.count + 1 : 1 }],
  ...
  const removeProduct = () => {
    if (cartItem) {
      upsertCart(
        [{ productId: product.id as string, count: +cartItem.count - 1 }],
        { onSuccess: invalidateCart }
      );
    }
  };
